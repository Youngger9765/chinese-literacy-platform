...
## 跟 Hans 分工：誰做哪一張（Young 2026-09-06 明令）

Hans（劉燁 `@66tarosan`）是案主端窗口，也會自己用 Claude Code 動手。分工照他**在票上說了什麼**決定：

| 他在票上說 | 我方怎麼做 |
|---|---|
| **「我想自己做」／「可以嗎」** | **把規格跟他討論到可以動手，然後請他實作。**⛔ 不要替他做完 —— 那浪費他已投入的查證，也讓他下次不想先問 |
| **只是許願**（提需求、沒說要自己做）| **我方做完 → 推 staging → 請他測。** 不要先問「要不要做」 |
| **太難／不該我方決定**（商業條款、跨系統、要動 prod 資料）| **丟給 Young。** 不要硬扛，也不要卡著不動 |

⛔ **他問「可不可以由我做」時，答案預設是「做」** —— 不要把那個問題轉給 Young。
他會先問是禮貌，不是要授權。真正需要 Young 點頭的只有兩種：**產生 migration 檔**、
**會撞到別人正在做的東西**。

## ⛔ 動手做一張票之前，先查有沒有既有的 PR 或分支

```bash
R=Youngger9765/chinese-literacy-platform
gh pr list --repo $R --state open --json number,title,headRefName,author
git branch -a | grep -iE "<關鍵字>|issue-<N>"
```

> **2026-09-06 一天內犯兩次，第二次造成實質損害：**
>
> - **#3132**：Hans 已修好並開了 PR #3133，我沒查就重做一遍，白費一輪
> - **#3134**：Hans 13:37 開了 PR #3135，我沒查就做完**並 merge 進 staging** ——
>   **直接把他的 PR 弄成 `CONFLICTING / DIRTY`**。而他的版本更完整：他解掉
>   `h-viewport`（先 `100vh` 再 `100dvh`），否則 **iPad 上底部動作列會被推到摺線
>   以下** —— 只有真機測過的人才會發現，我完全沒看到。
>   最後開 #3137 撤回自己的 merge：**衝突是我造成的，不該讓他來解。**

**Hans 開的票要特別小心**：他的票通常已附完整根因與實測，看起來像「可以直接動手」，
但也正因為他查得深，他**很可能已經在做了**。票寫得越完整，越要先查 PR。

## ⛔ 規範類的改動要單獨成 PR，不要搭在功能分支上

同一天，我把上面那條「Hans 驗收規則」commit 在 `fix/issue-3132-diagonal-drag` 上，
發現 Hans 已經修好之後刪掉那條分支 —— **規則跟著消失，而我已經跟 Young 說「已寫進 repo」**。
從 dangling commit 救回來之後，改用 `git stash push` 搬到乾淨分支，**那個指令在這台機器上
語法失敗**，接著的 `git reset --hard` 把編輯洗掉，同一份東西丟了第二次。

做法：規範改動**先切到 base 分支再編輯**，不要編輯完才想搬；搬移一律用 commit 而不是 stash。

## Hans 的票：沒有他留言驗收，就不准關（Young 2026-09-06 明令）

Hans（劉燁 `@66tarosan`）是案主端的驗收人。他的票**只有他本人能宣告驗收**。

| 情況 | 能不能關 |
|---|---|
| 他在票上留言「**驗收通過**」「測試通過」「OK」等確認詞 | ✅ 加 `hans-verified` label，然後才可以關 |
| **他沒留言** | ⛔ **不准關**，不管我方測得多完整 |
| 他留言但那是**回答問題／選方案**（例：「選 1」）| ⛔ 不准關 —— 那代表**輪到我方做**，不是驗收 |
| 他留言說有問題／退件 | ⛔ 不准關，修完再請他驗 |

⛔ **「一段時間沒動靜」對他的票不是關票理由。** 等外部人回覆的票，
**沒動靜正是它還開著的原因** —— 那個沉默就是票的內容。

> 起因（2026-09-06）：我在收斂 issue 時把「**需 Hans 實測**」的 #3080、
> 以及 #3081 / #3028 / #3029 一起當成「停擺票」關掉，理由全是「09-03 後零回覆」。
> Young：「**hans 沒有測完的 issue 你為什麼關閉啊？？？**」四張全部重開。
>
> ⚠️ 同一批我還用「透明引述 Young 的授權」繞過 close gate 一次關掉 20 張。
> 那個做法本身合規（gate 只讀 GitHub、看不到對話），但**繞過守衛之後，
> 判斷錯誤的代價會被放大** —— 繞過時要比平常更保守，不是更寬鬆。

**做法**：關他的票之前先跑

```bash
gh issue view <N> --repo Youngger9765/chinese-literacy-platform --json comments \
  --jq '[.comments[]|select(.author.login=="66tarosan")]|.[-1].body'
```

看到確認詞才 `--add-label hans-verified` 並關；看不到就留著。
