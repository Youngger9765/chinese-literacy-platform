...
def _structure_from_parsed_yaml(
    lesson_id: str,
    build_structure: Callable[[dict], dict | None],
) -> dict | None:
    path = PARSED_DIR / f"{lesson_id}.yml"
    if not path.is_file():
        return None
    import yaml

    data = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
    if not data.get("story_structure_table") and not data.get("story_structure_rows"):
        return None
    return build_structure(data)
...
def _live_structure(
    ...
    if loader:
        return build_structure(loader)
    return _structure_from_parsed_yaml(lesson_id, build_structure)
...
def verify_manifest_freshness(
    *,
    ...
    for entry in manifest.get("lessons") or []:
        ...
        if not loader:
            if entry.get("tier") == "docx_keypoints" and not (PARSED_DIR / f"{lesson_id}.yml").is_file():
                errors.append(f"{lesson_id}: docx_keypoints in manifest but no loader or parsed YAML")

        struct = _live_structure(lesson_id, loader, build_structure)
        if not struct:
            continue
