...
def check(uid: str) -> tuple[bool, list[str]]:
    ...
    if not v3:
        # ⚠️ 「沒有課文段落」有兩種，判準要分得開，否則會把一整個課型判成漏抄：
        #    (a) 有課文但漏抄了 —— v2 抽得到段落，v3 抽不到
        #    (b) 這個課型本來就沒有課文 —— 例如「會考圖文題實戰」只有一個大題，
        #        原稿裡連「讀全文／念順順／課文」這些字都不存在（L0136 文字層 431 字，
        #        內容全在圖上）。要求它交出課文段落，是要求它抽出教材沒有的東西。
        #
        # 分法：v2 也沒有段落 ⇒ 沒有東西可以「不見」。這比讀 sections_absent 可靠 ——
        # 後者是抽取者自己寫的宣告，用它當判準等於讓被檢查的一方決定要不要被檢查。
        if not v2:
            return True, []
        return False, [f"{uid}: v3 沒有課文段落，而 v2 有 {len(v2)} 段 —— 整節不見了"]
    if not v2:
