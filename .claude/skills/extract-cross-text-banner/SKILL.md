---
name: extract-cross-text-banner
description: 抽「跨文本橫幅」這一個模組 —— 照派工單只讀自己那幾頁，產出 cross_text_banner.yml 並通過它自己的 schema。⛔ 不抽其他模組。當需要「抽跨文本橫幅」「重抽 cross_text_banner」「派工單派到 cross_text_banner」時使用。來源 issue #2843。
---

# extract-cross-text-banner — 跨文本橫幅
...
s=json.load(open('specs/modules/schemas/cross_text_banner.schema.json'))
b=yaml.safe_load(open('\$OUT'))['cross_text_banner']
...
`scripts/skill_dryrun_diff.py --module cross_text_banner` 對**全部 2 課**做了一次
重抽對帳：把這個模組的每個逐字欄位從 DOCX 的 `<w:t>` 流（文件順序，不經排版）
重新取一次，跟現有 yml 逐字比對。

```
2 課 · 逐字一致 2 · 對不上 0 · 受檢 2 字串
```
