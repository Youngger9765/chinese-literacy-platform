---
name: extract-modern-translation
description: 抽「古文今譯」這一個模組 —— 照派工單只讀自己那幾頁，產出 modern_translation.yml 並通過它自己的 schema。⛔ 不抽其他模組。當需要「抽古文今譯」「重抽 modern_translation」「派工單派到 modern_translation」時使用。來源 issue #2843。
---

# extract-modern-translation — 古文今譯
...
s=json.load(open('specs/modules/schemas/modern_translation.schema.json'))
b=yaml.safe_load(open('\$OUT'))['modern_translation']
...
`scripts/skill_dryrun_diff.py --module modern_translation` 對**全部 10 課**做了一次
重抽對帳：把這個模組的每個逐字欄位從 DOCX 的 `<w:t>` 流（文件順序，不經排版）
重新取一次，跟現有 yml 逐字比對。

```
10 課 · 逐字一致 10 · 對不上 0 · 受檢 33 字串
```
