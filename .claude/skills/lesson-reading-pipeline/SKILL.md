...
一課教材進來後，「朗讀」這條線的必做項只有一件：**① 抽出教授指定的段落**。

```
二修 DOCX ──①──> key_reading.yml（教授指定的那一段）        ← 必做
                      │
                      └──②──> 預生成音檔 → GCS              ← 目前不需要，見 §②
```

> **② 預生成在 2026-08-18 決定不做**（靖杭實測）。08-08 換成 Azure（#2616）之後，
> 現場合成約 **3 秒**就出聲，不再是 #2605 記的 8–24 秒 —— 那是 Gemini 3.1 Flash TTS
...
**先量，再決定要不要跑。** 判準是使用者按播放到出聲的時間，不是覆蓋率數字：

| provider | 現場合成延遲 | 預生成有必要嗎 |
|---|---|---|
| `azure`（**現行**，#2616 起） | 約 **3 秒** | ❌ 不需要 |
| `gemini31`（2026-08-08 前） | 8–24 秒，偶發 503 | ✅ 需要 |

`azure/sentences/` 現在對執行期的覆蓋率是 **29/8679（0%）**，而且體驗是可接受的 ——
**覆蓋率低本身不是問題**。這一點反直覺，所以寫在這裡：#2605 的症狀是「等 8 秒」，
根因是快取不可達，但**換掉 provider 也解掉了症狀**，於是根因不再值得花錢修。

下面的內容在兩種情況下才需要：**(a)** 改了 `normalization.py` 的發音修正表
（全庫 key 會輪替，若那時仍依賴快取就得重生），**(b)** 換回 Gemini 或其他慢的 provider。
...
- ❌ 看到「涵蓋率 0%」就去跑整批生成 → 先量現場合成要多久。Azure 下 3 秒，跑了是白花 NT$186
- ❌ 拿 #2605 的「8–24 秒」當現況 → 那是 gemini31 時代；#2616 換成 Azure 後不成立
- ❌ 相信文件寫的 provider 而不去看 deploy workflow → `docs/ops/ai-reading-batch.md` 寫「皆為 gemini31」，三個 workflow 實際都是 `azure`，我照它跑了 55 分鐘生到沒人讀的 prefix
