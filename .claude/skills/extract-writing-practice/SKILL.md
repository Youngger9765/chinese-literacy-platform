---
name: extract-writing-practice
description: 抽「語詞書寫練習」這一個模組 —— 照派工單只讀自己那幾頁，產出 writing_practice.yml 並通過它自己的 schema。⛔ 不抽其他模組。當需要「抽語詞書寫練習」「重抽 writing_practice」「派工單派到 writing_practice」時使用。來源 issue #2843。
---

# extract-writing-practice — 語詞書寫練習
...
```
4 / 175 課有
核心欄位（≥90% 課都有）：columns, instruction, words
```
...
s=json.load(open('specs/modules/schemas/writing_practice.schema.json'))
b=yaml.safe_load(open('\$OUT'))['writing_practice']
...
`scripts/skill_dryrun_diff.py --module writing_practice` 對**全部 4 課**做了一次
重抽對帳：把這個模組的每個逐字欄位從 DOCX 的 `<w:t>` 流（文件順序，不經排版）
重新取一次，跟現有 yml 逐字比對。

```
4 課 · 逐字一致 4 · 對不上 0 · 受檢 8 字串
```
