...
### 1. 抽取 raw table（保留合併）
跑 `scripts/extract_docx_blocks.py <docx路徑> --tables-only`。對重點表那張 table，輸出每 cell 的 `(row, col, text, gridspan, vmerge)`，**不要先攤平**。
重點表 = 含 問題/解決/結果/主角/主題/事例/上位概念/元素 標籤、且帶 `【 】` 的那張（通常在聚光燈之後、MCQ 之前）。

### 2. 還原層級
- 左欄連續相同 label（合併）→ 該 label 是父節點，右側多列是 `sub_rows`。
