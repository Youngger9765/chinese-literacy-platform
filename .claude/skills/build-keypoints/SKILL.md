...
### 服務層現在怎麼接（#2776，2026-08-20 補）

上面「句子留在同一列」的形狀對了，但**消費端一度沒接住**：`cell_to_structure_fields`
（`story_structure_cell_parser.py`）的 checkbox 偵測只要看到 □ 加圈號，就把**整段文字**
當成一組 checkbox——兩組「第N個空格」的選項被攤平合併（`①贏了②輸了①贏得②失去` 4 個
選項混一組），而且**句子本身被丟掉**，畫面上只剩 4 個孤立的勾選框，比原本什麼都不顯示
更難懂（跟展開成「X-1」「X-2」是同一族失敗，只是換了個地方發生）。

現在的接法：`parse_inline_choice_groups()` 在 checkbox 偵測**之前**先認一次「第N個空格」
的行，抓到 ≥2 組就轉成新的 `interactive_type: inline_choice`：

```yaml
interactive_type: inline_choice
value: "結果，小戴【　　　】球賽，卻【　　　】全國人民的尊敬。"   # 句子保留，"第N個空格" 說明行已移除
blanks:
  - options: ["贏了", "輸了"]       # 第一個空格自己的選項
    correct_option: 1              # 伺服器端才有，消毒後不送給學生
  - options: ["贏得", "失去"]       # 第二個空格自己的選項
    correct_option: 0
```

前端 `InlineChoiceContent` 在句子裡每個真空格的位置插一個小選項按鈕群組
（`InlineChoicePicker`），不是把選項印在句子後面。抽取階段**不需要改任何東西**——
只要照本節開頭寫的「句子 + 第N個空格：①…②…」格式輸出，服務層會自動轉換。

⚠️ **只在「句子裡每一個真空格都恰好對應一組『第N個空格』」時才會轉換**；如果句子裡
還混了一個沒有選項、純自由文字的空格（如 L0102「…重金屬根本【　】」跟後面兩個選擇題
空格混在同一格），空格數與選項組數對不上，會**安全地維持原本的 checkbox 行為**（不
會硬套錯誤的對應關係）。這種「一格裡混自由文字空格跟選擇題空格」的複合形狀，
建議抽取時**拆成兩個獨立小題**（一個 fill_blank + 一個/多個 inline_choice 候選），
不要塞進同一個 cell——现行服務層還沒有辦法安全地拆開已經混在一起的資料。
...
### 「單選」不是只給人看的說明文字（#2776，2026-08-20 補）

抽取時寫 `(單選，請打勾)` / `(多選，請打勾)` 不是白寫的——服務層現在會讀它，把
checkbox 列標成 `select_mode: "single"` 或 `"multi"`，前端用這個欄位決定要不要
擋掉複選。**沒寫指示語的舊形狀** `select_mode` 會是 `None`，前端維持原本可複選的
行為（不會突然變嚴格）。抽取新課時，只要學習單上寫了「單選」兩個字，照抄進
value/label 即可，不用額外做什麼——別自己刪掉這句話讓畫面「看起來乾淨」。

### `(1)` `(2)` 段落編號不是要挖的空格（#2776，2026-08-20 補）

服務層的括號正規化（把 `（答案）` 轉成 `【答案】`）以前**連段落編號也轉**——
`(1)棉花肺實驗的問題` 變成一格預先填好答案「1」的填空欄，而且同一 row 裡多個
子題常常共用同一個被抹空的標籤（`研究一【　　　】` 出現兩次，分不出哪個是哪個）。
現在已經排除純 1-2 位數字的括號內容，抽取時**不需要**為了閃避這個問題而改寫
段落編號的寫法（`(1)` 這種寫法本來就是對的，照抄）。
...
- ❌ **抽取時把「單選」指示語刪掉或改寫**（服務層靠它判斷 `select_mode`，刪掉會讓
  該選一個的題目變成能複選，#2776）
- ❌ **把一個自由文字空格跟選擇題空格塞進同一個 cell**（如「…根本【　】」後面接著
  兩個「第N個空格」選擇題）——服務層無法安全拆開，抽取時就分成兩個獨立小題
