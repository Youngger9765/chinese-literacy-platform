...
**已做過全庫重抽對帳（見文末實跑紀錄），但尚未逐題重抽。** 上面的數字全部來自對現有 70 課 99 條的統計，
...
`scripts/skill_dryrun_diff.py --module errata` 對**全部 70 課**做了一次
重抽對帳：把這個模組的每個逐字欄位從 DOCX 的 `<w:t>` 流（文件順序，不經排版）
重新取一次，跟現有 yml 逐字比對。

```
70 課 · 逐字一致 69 · 對不上 1 · 受檢 64 字串
```
...
```
errata 的 source   …早期豆腐製成的素肉，，最強調哪一項改進？
原稿               …早期豆腐製成的素肉，最強調哪一項改進？
```

**原稿只有一個逗號**（`word/document.xml` 逐字確認，`norm()` 不會壓掉疊字標點，
這點也驗過了）。所以那條勘誤記了一個**不存在的錯字** —— 要嘛它是對著更早的
原稿版本寫的、後來案主改掉了，要嘛當初轉錄錯。

⛔ 這種假勘誤比沒有勘誤更糟：會讓人去「修」一個本來就對的地方。
已回報 → #2874。
