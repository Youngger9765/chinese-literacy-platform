...
### ⑥.68 念順順的朗讀範圍＝**指定的那一段，就那一段**

```yaml
key_reading:
  start_paragraph: 4
  end_paragraph: 4        # 恆等於 start_paragraph
  passage: (第 4 段的全文)
```

2026-07-20 教授審查定的規則，`backend/data/key_reading_passages.yml` 檔頭也記著：
**☞ 那一段 → 學習單右緣累計字數欄末筆落在的那一段，兩端之間全包**（2026-08-30 改，Owner override）。非全文、非「☞→結尾」——累計欄常常提早停，停在哪就到哪。完整理由見 `lesson-reading-pipeline`。

⛔ **不要用右緣累計字數欄去決定終點段。** 那一欄是「一分鐘能讀到哪」的量尺
（實測 max 落 280–520，而課文本身 535–1670 字），與朗讀範圍無關 —— 下一節說「不要拿它
反推段界」，決定終點段是同一件事的另一個說法。

⛔ **也不要寫 `spans_paragraphs` / `approx_chars_from_start`。** 前者暗示範圍是多段，
後者就是字數欄 max。兩個欄位放在單段 passage 旁邊，會讓下一個人從它們重建出範圍規則。

**這條規則被繞過四次**，每次都是同一個形狀（用推論覆蓋教材上的標記）：

| | 規則 | 中位字數 | 對一版人工掃描（38 課可判定） |
|---|---|---|---|
| 最早 pilot | ☞ → 全文結尾 | — | 被否決 |
| #2712 | ☞ → 累積到 max(累計字數) | 370 | 被否決 |
| v3 初版 | ☞ → 累積到 max(累計字數) | 391 | 被否決 |
| 範圍版（#2913） | ☞ → 字數欄末筆落在的那一段 | 383 | **2 / 38**，靖杭看原稿否決 |
| ~~2026-08-24~~ | ~~只取那一段~~ | ~~148~~ | ~~36 / 38（**一版**黃金集）~~ |
| **現行（2026-08-30）** | **☞ → 累計欄末筆** | **382** | 對帳貼合率 **85%**（二修學習單）|

⚠️ 判定方法要寫清楚，不然數字會互相矛盾：取一版人工掃描的段落，保留「該段文字在二修
**恰好一課**裡逐字存在」的 38 筆，再比**整段相等**。用「包含」比會虛高 —— 一個 390 字
的範圍當然「包含」正確的那一段，但它仍然是錯的（同一組樣本，包含比會把範圍版報成 9/38）。
...
1. **段落可以合法地以「：」結尾** —— 「這裡的推理三要素是：」、「然後，這首歌出現了：」、
   「例如：」後面接的是各自編號的段落。若把「補完句子」的條件只設 `。！？」』…`，
   這三課會各自把下一段吃進來，就不再是「那一段」了。冒號要算句尾。
2. **指定段可能只有 11 字**（L0140 第十三段「這個故事有三個大轉折。」）。
   短≠錯，**不可以加長度下限把它擋掉** —— 一版教授自己標的段落就有 19 字的。
   照規則寫，然後標 `needs_human_review` 讓人看，不要吃掉也不要假裝沒事。

**誠實標的欄位寫在檔案最上層**（不是 `key_reading:` 裡面），與 `build_lesson_body.py`
一致，`test_lesson_uid_loader.py` 也在那裡讀：

```yaml
extraction_check: {verdict: short_marked_paragraph, corroborated_by_first_edition: null}
needs_human_review: true
review_reason: 學習單指定的那一段只有 11 字（第13段）。…要人看紙本確認段號沒讀錯。
key_reading:
  passage: …
```

⚠️ 因此這支的正確性依賴 §⑥.55 的 `idx` 真的是印刷段號。若哪次抽取把段落合併或重編，
它會安靜地跟著錯 —— 所以輸出一律附上與一版人工掃描的逐字比對（`confirmed` /
`disagrees_with_first_edition`），不要拿掉那個欄位。
