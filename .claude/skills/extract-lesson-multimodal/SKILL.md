...
`→` 可能是 `<w:sym w:font="Wingdings" w:char="F0E0"/>`、`➜`／`➤` 同理，
圈號 `①②③` 也可能是。**它們不在 `w:t` 裡**，所以你把 `A→B` 照字面寫進 yml，
逐字門拿去跟原稿比就對不上 —— 而那不是你抄錯，是原稿根本沒有那個字元。

→ 做法：**在箭號前後斷行**（HTML 會收成空格，畫面仍是一行），不要寫入字面箭號。
L0031 的既有寫法就是這樣；L0122 沒照做，3 處逐字門 FAIL。
