---
name: extract-classical-text
description: 抽「原文（含斷詞點）」這一個模組 —— 照派工單只讀自己那幾頁，產出 classical_text.yml 並通過它自己的 schema。⛔ 不抽其他模組。當需要「抽原文（含斷詞點）」「重抽 classical_text」「派工單派到 classical_text」時使用。來源 issue #2843。
---

# extract-classical-text — 原文（含斷詞點）
...
```
10 / 175 課有
核心欄位（≥90% 課都有）：annotations, paragraphs
```
...
s=json.load(open('specs/modules/schemas/classical_text.schema.json'))
b=yaml.safe_load(open('\$OUT'))['classical_text']
...
`scripts/skill_dryrun_diff.py --module classical_text` 對**全部 10 課**做了一次
重抽對帳：把這個模組的每個逐字欄位從 DOCX 的 `<w:t>` 流（文件順序，不經排版）
重新取一次，跟現有 yml 逐字比對。

```
10 課 · 逐字一致 10 · 對不上 0 · 受檢 68 字串
```
