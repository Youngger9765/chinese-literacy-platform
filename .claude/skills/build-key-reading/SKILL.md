...
# ⚠️ 二修（2026-08）：錨點的載體變了，上面那套 ☞ 圖形定位已不適用

上面第 1 節寫的是**一修**的機制 —— DOCX 裡一個 ☞ 手指圖形**直接指著**那一段，所以不需要任何座標系統。

**二修改成文字指令**：

> 請用計時器，**從指定段落（四）開始朗讀**，計時 1 分鐘讀的字數，並記錄在表格中。

錨點因此從「一個指標」變成「**一個序數**」，而序數必須有座標系才有意義。這一步是二修所有取錯段的根源。

## 這裡最容易犯的錯：拿錯的清單去數

```python
passage = body[anchor - 1].strip()      # ⛔ 這行是 bug 的來源
```

`body` 是 `scripts/extract_lesson_body.py` 用一整串啟發式**推導**出來的清單，目的是「**要顯示什麼課文**」：

- `_MIN_BODY = 46` 字長度門檻
- 短段落只在長段落旁邊才撿回（`_SHORT_RUN_LIMIT`）
- chrome / instruction / exercise 前綴排除
- `normalise()` 去重

而印刷段號回答的是完全不同的問題：「**老師數的是什麼**」。

**兩個座標系目標不同，卻共用同一份清單** —— 任何一條規則在某一課多留或少留一段，那一課之後的每一個序數就整體位移一格。

## 已證實的兩種位移（來源：#2720 逐課比對，13 課確定錯）

### 一、短引言被長度門檻丟掉 → 之後每個序數少一格（Δ=+1，7 課）

`L0004`：印刷的第 1 段是 22 字的引言

```
    最近心情起起伏伏，變得好不習慣，好不像我……      ← 印刷第 1 段，我們丟掉了
    上課時，我明明想專心聽老師講解…                  ← 印刷第 2 段 = 我們的第 1 段
    然而最近我更發現當他和其他同學聊天…              ← 印刷第 3 段 = 我們的第 2 段  ← anchor 指這裡
```

anchor 說「3」，我們取自己的第 3 段（我變得很敏感…），老師要的是自己的第 2 段。

### 二、Word 把一段切成兩個 `<w:p>` → 只服務半段，句子沒講完（B 組 2 課）

`L0017`：

```
    body 各段字數: [59, 98, 96, 119, 176, 24, 87]
                                       ↑    ↑
                                anchor=5   「有多一些互動，我就會覺得這一天好像變得特別一點。」
```

第 6 段那 24 字是第 5 段的尾巴。老師的一段 = 176 + 24 = 200 字，我們只給前 176 字。

## 修法方向：錨點要有自己的清單

**不要**去調 `extract_lesson_body.py` 的門檻來遷就錨點 —— 那會為了一個目的破壞另一個目的（課文顯示會開始出現不該出現的短行）。

正確做法是**為錨點另建一份「印刷段落」清單**，它的規則跟顯示用的 body 不同：

| | 顯示用 body | 印刷段落清單（錨點用） |
|---|---|---|
| 目的 | 學生看到的課文 | 老師數的第 N 段 |
| 短行 | 依門檻可丟 | **一律保留**（印刷上它就是一段） |
| 被 Word 切開的段 | 兩段 | **合併回一段** |

## ⛔ 不要做的事

- **不要改「只取標記那一段」這條規則**。那是 2026-07-20 教授決策。我犯過：把它改成「累積到 300 字讓學生念滿一分鐘」，中位數從教授的 153 字變成 370 字、60/67 課超過 1.5 倍，並且我還寫了一條「至少 120 字」的測試把這個錯誤鎖起來
- **不要用「段落結尾沒有句末標點」當合併判準**。全體有 77 段符合、影響 50 課，但裡面混著真的斷句（`L0008` 的「拿出瓶」＋「子喝了口水」）、圖說（`L0002`）、歌詞引文（`L0007`）—— 這個訊號會誤傷
- **不要用「在 `<w:tbl>` 內就不是課文」**。`L0149`／`L0146`／`L0102` 整篇課文都在表格裡（8/8、9/9、11/11 段）

## 怎麼驗（兩個獨立來源，都不必人工核對）

**一、一修人工掃描真值** —— `重點朗讀文件/`，與 `backend/data/key_reading_passages.yml` 逐字一致。
⚠️ 只能覆蓋 30 課（13 錯 + 17 對），另外 116 課因為二版改寫課文或標題對不上而**沒有基準** —— 隨機抽查很可能全部落在有基準的那 17 課，看起來像沒問題。**主張範圍不要超過可比對的子集。**

**二、每分鐘字數目標（#2722，全部 154 課都有）** —— 教材第二節印著這一課的目標（`□ ＜190字 / □ 191~220字 / □ ＞221字`）。硬性關係：

> **段落長度必須 ≥ 分級下限**，否則學生念不到一分鐘就沒字念了。

現況 29/146 通過。這個判準覆蓋面比一修真值大得多，而且完全機器可驗，適合當回歸鎖。

## 相關 issue

- `#2712` 範圍過長（已修：改回只取標記那段）
- `#2720` 取錯段（印刷段號 vs 陣列索引）
- `#2722` 每分鐘字數目標沒送到前端
- `#2724` 只帶目標沒有段落的 key_reading 造成 11 課 500
