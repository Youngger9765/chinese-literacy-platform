---
name: extract-key-reading
description: 抽「念順順 / 重點朗讀」這一個模組。⚠️ 內容規格不在這裡 —— 在 `lesson-reading-pipeline`，先讀那一份。這一份只補派工層的介面契約（照派工單只讀自己那幾頁、schema 自驗、過對帳門）。當需要「抽念順順 / 重點朗讀」「重抽 key_reading」「派工單派到 key_reading」時使用。來源 issue #2843。
---

# extract-key-reading — 轉接，不是規格

## 🔴 內容規格在別的地方，先讀那一份

```
.claude/skills/lesson-reading-pipeline/SKILL.md   （305 行）
```
...
**維護者：靖杭 @if-else-master（重點朗讀那個 PR）。** ⛔ 內容規格要改就改那一份，不要改這裡。

## 這一份存在的唯一理由

派工單（`_manifest.yml`）派工時印的是「對每一架跑 **extract-<module>** skill」。
`key_reading` 這個名字以前解不到任何東西 —— 操作的人只能自己去猜是哪一支，
...
## 派工層的介面契約（lesson-reading-pipeline 那一份寫在 #2843 之前，沒有這一段）
...
```bash
python3 scripts/witness_reconcile_gate.py --uid <UID> --module key_reading \
...
**這是轉接層，本身沒有實跑紀錄。** 實跑狀況看 `lesson-reading-pipeline` 那一份。
...
`scripts/skill_dryrun_diff.py --module key_reading` 對**全部 157 課**做了一次
重抽對帳：把這個模組的每個逐字欄位從 DOCX 的 `<w:t>` 流（文件順序，不經排版）
重新取一次，跟現有 yml 逐字比對。

```
157 課 · 逐字一致 157 · 對不上 0 · 受檢 1517 字串
```
