---
name: extract-full-text-annotate
description: 抽「讀全文-做記號」這一個模組 —— 照派工單只讀自己那幾頁，產出 full_text_annotate.yml 並通過它自己的 schema。⛔ 不抽其他模組。當需要「抽讀全文-做記號」「重抽 full_text_annotate」「派工單派到 full_text_annotate」時使用。來源 issue #2843。
---

# extract-full-text-annotate — 讀全文-做記號
...
```
164 / 175 課有
核心欄位（≥90% 課都有）：char_count, inline_marked_terms, paragraph_count, paragraphs
```
...
s=json.load(open('specs/modules/schemas/full_text_annotate.schema.json'))
b=yaml.safe_load(open('\$OUT'))['full_text_annotate']
...
`scripts/skill_dryrun_diff.py --module full_text_annotate` 對**全部 164 課**做了一次
重抽對帳：把這個模組的每個逐字欄位從 DOCX 的 `<w:t>` 流（文件順序，不經排版）
重新取一次，跟現有 yml 逐字比對。

```
164 課 · 逐字一致 164 · 對不上 0 · 受檢 3049 字串
```
...
## 🔴 實跑撞到（2026-08-23，L0011）：版面上看得到但沒收

漏了三欄，都在頁面上：

```
reading_tip           文章上方的提示語（「讀文章時，隨時留意自己的理解和感受…」）
inline_marked_terms   句中被標記的語詞（防禦 / 摸不著頭緒 / 龍爭虎鬥 …）
marking_instruction   做記號的指示（※ 如果你有做到下列事項，請在□內打勾。）
```

⚠️ schema 過、逐字門過 —— **兩道門都看不出「少抽了東西」**，
因為它們只檢查已經寫下來的東西對不對。
