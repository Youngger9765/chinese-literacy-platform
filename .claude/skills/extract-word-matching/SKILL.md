---
name: extract-word-matching
description: 抽「文白詞語比對」這一個模組 —— 照派工單只讀自己那幾頁，產出 word_matching.yml 並通過它自己的 schema。⛔ 不抽其他模組。當需要「抽文白詞語比對」「重抽 word_matching」「派工單派到 word_matching」時使用。來源 issue #2843。
---

# extract-word-matching — 文白詞語比對
...
```
11 / 175 課有
核心欄位（≥90% 課都有）：instruction, items, notes
```
...
s=json.load(open('specs/modules/schemas/word_matching.schema.json'))
b=yaml.safe_load(open('\$OUT'))['word_matching']
...
`scripts/skill_dryrun_diff.py --module word_matching` 對**全部 11 課**做了一次
重抽對帳：把這個模組的每個逐字欄位從 DOCX 的 `<w:t>` 流（文件順序，不經排版）
重新取一次，跟現有 yml 逐字比對。

```
11 課 · 逐字一致 11 · 對不上 0 · 受檢 165 字串
```
