...
| [**前端設計.md**](./前端設計.md) | 32 KB | React + TypeScript + TailwindCSS<br>學生端 (Dashboard + 作業提交 + 錄音)<br>教師端 (批改介面 + 數據分析)<br>**Admin UI (課程管理介面)**<br>AudioRecorder 組件<br>React Query 狀態管理 | 19 天 |
| [**AdminUI課程管理設計.md**](./AdminUI課程管理設計.md) | 28 KB | **方案 H: GitHub + Admin UI**<br>TipTap WYSIWYG 編輯器<br>Git Service 整合層<br>Markdown 轉換器<br>版本歷史與回復<br>自動儲存與衝突處理 | 12 天 |
...
**基礎平台開發成本**: $27,500 (以 $110/hour 計算)

#### Admin UI (可選,方案 H)

| 階段 | 時程 | 人力 |
|------|------|------|
| 後端 API + Git Service | 3.5 天 | 1 後端工程師 |
| 前端 Admin UI | 5.5 天 | 1 前端工程師 |
| 測試 | 1 天 | 1 QA 工程師 |
| **小計** | **2.5 週 (並行)** | 3 人團隊 |

**Admin UI 開發成本**: $10,560
...
### 階段 2: Admin UI (可選,2.5 週)

| 週次 | 後端 | 前端 | 測試 |
|------|------|------|------|
| **W7** | Git Service + Markdown 轉換器 | 課程列表 + 基礎編輯頁面 | Unit Tests |
| **W8** | 版本歷史 API | TipTap 編輯器 + 生字管理 | Integration Tests |
| **W9** | 衝突處理 | Auto-save + 版本比對 | E2E Tests |
...
- **總大小**: ~410 KB
- **完成日期**: 2026-02-13
- **設計時程**: 持續迭代 (從外部系統評估 → API 驗證 → 最終設計 → Admin UI 補充)
