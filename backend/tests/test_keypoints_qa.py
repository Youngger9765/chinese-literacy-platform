"""Route tests for keypoints QA save/list/load (GCS-only, no DB).
...
Run:
    cd backend && python -m pytest tests/test_keypoints_qa.py -v
"""
...
from app.routes.keypoints_qa import _PATH_RE, _slug

client = TestClient(app, raise_server_exceptions=False)

GOOD_PAYLOAD = {"reviewer": "啟翔", "tool_version": 1, "lessons": [{"lesson_code": "G6-L22"}]}
BUCKET = "app.routes.keypoints_qa._get_gcs_bucket"
...
class TestSave:
    def test_save_happy_path(self):
        ...
        assert body["path"].startswith("keypoints-qa/")
    ...
    def test_missing_lessons_400(self):
        with _non_prod(), patch(BUCKET, return_value=_mock_bucket()):
            r = client.post("/api/keypoints-qa/save", json={"reviewer": "x"})
        assert r.status_code == 400

    def test_too_large_413(self):
        big = {"reviewer": "x", "lessons": [{"blob": "A" * (5 * 1024 * 1024)}]}
        with _non_prod(), patch(BUCKET, return_value=_mock_bucket()):
            r = client.post("/api/keypoints-qa/save", json=big)
...
class TestListAndRead:
    def test_reviews_parses_path(self):
        blob = MagicMock()
        blob.name = "keypoints-qa/啟翔/20260707T101112-abcd1234.json"
        blob.size = 123
        b = MagicMock()
        b.list_blobs.return_value = [blob]
        with patch(BUCKET, return_value=b):
            r = client.get("/api/keypoints-qa/reviews")
    ...
    def test_review_round_trip(self):
        path = "keypoints-qa/啟翔/20260707T101112-abcd1234.json"
        blob = MagicMock()
        ...
        with patch(BUCKET, return_value=b):
            r = client.get("/api/keypoints-qa/review", params={"path": path})
        assert r.status_code == 200
        assert r.json()["reviewer"] == "啟翔"

    def test_review_path_traversal_rejected(self):
        for bad in ["spotlight-qa/x/y.json", "keypoints-qa/../secret.json", "keypoints-qa/a/b.txt"]:
            r = client.get("/api/keypoints-qa/review", params={"path": bad})
    ...
    def test_slug_charset_aligned_with_path_re(self):
        ...
        for name in ["啟翔", "José", "さくら", "김", "王 小明", "a-b/c", "", "🚀only"]:
            path = f"keypoints-qa/{_slug(name)}/20260101T000000-abcd1234.json"
