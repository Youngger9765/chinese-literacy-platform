...
## 已經因為「樣式比對沒有詞界」而修過的字（同一型，不要重蹈）

| 字 | 票 | 誤中的形狀 | 處置 |
|---|---|---|---|
| 和 | #3238 / #3246 / #3252 | 例外清單子字串命中（濕度/和氣/壓） | 改成對齊斷詞邊界＋補繁體鄰詞 |
| 兒 | #3264 | `poyin_db` 的輕聲樣式 `個*` 命中 `個/兒童`、`個/兒子` | 移出 `個*`（語料 8 處全誤中、0 真用例） |

⭐ **這一類的共同點**：樣式是「前字 + 目標字」的子字串比對，**沒有檢查目標字的右邊界**。
   所以只要目標字後面還接著一個能跟它成詞的字，就會被前面那個樣式搶走。
   ⚠️ 發現「兒」那一處的方式**不是測試也不是複審**，是**對教育部辭典抽樣量正確率**
   （`moedict.tw`，repo 裡 `dictionary_service.py` 本來就在用）—— 120 種 (字,槽) 組合裡
   它是唯一一個「不是該字合法讀音」的。這個量測值得變成常設的門。
