...
INJECTION = "BUILD_SHA=${{ github.sha }}"
...
def test_every_backend_deploy_workflow_injects_build_sha() -> None:
    ...
    for p in files:
        ...
        elif not any(INJECTION in l for l in block):
            missing.append(p.name)

    assert not missing, (
        f"{missing} deploy the backend without injecting {INJECTION!r} on the "
        f"backend's own deploy step.  A SHA that only reaches prod leaves those "
        f"environments unverifiable; one that lands on the frontend step reaches "
        f"the wrong container — see #3310."
    )
...
def test_injection_rides_on_the_env_var_flag_not_a_stray_comment() -> None:
    ...
    for p in _workflows_deploying_backend():
        ...
        on_a_flag = any(
            INJECTION in line and any(f in line for f in ENV_FLAGS) for line in block
        )
        assert on_a_flag, (
            f"{p.name}: {INJECTION!r} is not on an env-var flag within the backend "
            f"deploy step, so it never reaches the backend container"
        )
