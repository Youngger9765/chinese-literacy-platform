...
def _require_owner(classroom: Classroom, current_user: User) -> None:
    """Raise 403 if the current user is not the classroom's teacher."""
...
def create_classroom(
    ...
    """Create a new classroom. teacher_id is taken from the JWT token."""
    ...
    logger.info(
        "Created classroom %d '%s' for teacher %d in school %d",
        classroom.id, classroom.name, current_user.id, payload.school_id,
    )
...
def get_classroom_detail(
    ...
    _require_owner(classroom, current_user)
