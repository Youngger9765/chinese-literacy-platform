"""文章重點表 QA Board 儲存 (人工測試用) — GCS-only, no DB.

比照聚光燈 QA (spotlight_qa.py) 的隔離模式:公開靜態頁 (frontend/public/keypoints-qa/)
把一包 QA review JSON 打到這裡,存進 GCS 獨立 prefix,供人工審查用。

設計(為何 GCS-only 無 DB):
- alembic 目前 multi-head,依專案鐵律不在此情況下新建 migration。
- 每筆 review 存 GCS: keypoints-qa/{reviewer_slug}/{ts}-{uid}.json
- 與學生 attempt、testset 音檔、spotlight-qa 皆完全隔離(獨立 prefix),重用既有 reading-audio bucket。

隔離 / 不影響正式環境:
- 獨立 GCS prefix `keypoints-qa/`,不進 DB。
...
router = APIRouter(tags=["keypoints-qa"])

_PREFIX = "keypoints-qa"
...
async def keypoints_qa_save(request: Request):
    ...
    try:
        ...
        logger.warning("keypoints-qa save failed: path=%s err=%s", path, exc)
        raise HTTPException(503, "save failed, please retry")

    logger.info("keypoints-qa save OK: %s (%d bytes)", path, len(raw))
...
def keypoints_qa_reviews():
    ...
    try:
        ...
        logger.warning("keypoints-qa list failed: %s", exc)
...
def keypoints_qa_review(path: str = Query(...)):
    ...
    try:
        ...
        logger.warning("keypoints-qa read failed: path=%s err=%s", path, exc)
