...
### 🔴 P0-b 那 20 支紅的鎖，逐支判「真 regression 還是過時期望」

⛔ **不准為了讓它綠而改斷言** —— 那正是它存在要防的事。
每支三選一，且要寫理由：

1. **真 regression** → 修 code，鎖維持原樣
2. **期望過時**（資料/契約合法地變了）→ 改鎖，**並在 commit 說明什麼變了、為什麼合法**
3. **鎖本身沒有價值** → 刪（見 §3 判準）
...
每一種「學生做完之後應該留下的東西」各要一條：
分數 ✅（今天補）· 徽章 ✅（今天補）· 進度 key · 作業提交 · 學習紀錄可讀回
