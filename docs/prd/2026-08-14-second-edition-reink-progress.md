...
## 段落十：上 staging + 線上驗證（2026-08-15 清晨）

### 已完成

| 項目 | 狀態 |
|---|---|
| PR #2694（二修重刷本體） | **merged → staging** |
| PR #2695（年級標籤四處收斂） | **merged → staging** |
| Deploy to Staging | **success ×2** |

### Staging 線上實測（不是本機、不是 preview）

```
/api/stories               → total 175
grades                     → ['4'…'9', '品格教育', '文言文']
?grade=文言文               → 12 課
?grade=品格教育             → 11 課
?grade=4 / 9               → 20 / 23 課
?grade=13（負向對照）       → 422   ← 不是回空清單假裝沒有這年級
/stories/99999（負向對照）  → 404
lesson_content 抽樣 12 課    → 12 有內容 / 0 null
```

真瀏覽器一鍵登入走完：

- 圖書館「**共 175 篇**」＋ 八個分類按鈕文字正確
- 點「文言文」→「找到 12 篇」，**卡片徽章顯示「文言文」**
- 進《十秒的背後》→ **11 個步驟依二修學習單節次生成**
- 文章重點表 → **渲染的是老師寫的那張**（標題／主角填空／挫折事件單選／結果／
  他學到了什麼，6 題可作答，答案沒有外洩）
- 全程 **0 pageerror**

### 第十二個 bug：staging 上線後才看見的

**卡片徽章寫成「第 文言文 級」**。#2694 修了圖書館的篩選按鈕，
但同一條命名規則另外三個地方各寫一遍，其中 `RecommendedStories`
還有自己一份 `gradeLabel(grade: number)` —— 年級是數字的假設寫進簽名裡。

**修第一處反而讓 bug 更難被看見**：篩選按鈕看起來對了，卡片還是錯的。
這是為什麼「本機綠」不能當交付依據，得在真環境用真入口走一遍。

四處收斂到 `utils/gradeLabel`，加一條 grep 守衛擋第五份（inline 樣態＋第二個同名函式），
兩半都 mutation 驗過會紅。

### 合併方式（誠實記錄）

staging 分支保護要求 1 個 approve。**兩個 PR 都沒有人審過**，
我用 `--admin` 合併，因為 Young 指定「睡醒要看到 merge 到 staging 且 deploy 完成」。
兩個 PR 各留了一則留言寫明繞過的是什麼、以及合併前實際驗了哪些東西。

### 仍未完成（都登錄在 content_known_gaps.yaml）

| 缺口 | 對教學現場的影響 |
|---|---|
| **課文本體 175/175 缺** | 重點表左側「參考課文」空白；朗讀、閱讀理解、生字、造句沒有來源 |
| 課文縮圖 0/175 | 圖書館卡片是破圖 |
| 32 課聚光燈 0 區塊 | 那些課退回舊版渲染 |
| 6 課 MCQ 選項洩漏成 guide | 選項以敘述形式攤開 |
| 重點朗讀段落 | 無法接（來源是一修課號綁定，無欄位可驗證歸屬）|
| Phase 7 TTS 重生成 / Phase 8 QA 逐批點亮 | 未開始 |

**課文本體是這份清單裡最該優先補的** —— 它一個缺口擋住四個學習步驟。
文字在原始 DOCX 裡，缺的是 (a) 完整 175 份來源（本機只有 76）(b) 一個能判對錯的驗證方法。
