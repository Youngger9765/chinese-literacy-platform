...
│  手上這份 PDF 的頁數 ≠ 派工單的 pdf_pages → ⛔ 不派工       │  （②不穩的止血）
│                                                          │
│  ⚠️ **只比頁數不夠**（2026-08-22 實證）：L0001 兩次轉檔    │
│     都是 8 頁，但標題從「三  語詞我最棒」變成             │
│     「三 🅐 語詞我最棒」—— ⑤ 放行，而抽取範圍已經變了      │
│     架構師的建議是改用內容錨點，見 §0.5 Q1                 │
└──────────────────────────────────────────────────────────┘
                            │
    ┌───────────┬───────────┴───────────┬───────────┐
    ▼           ▼                       ▼           ▼
...
│  🔴 **抓到第一個真缺陷**：L0066 語詞應用漏抽第 8 題 → #2867│
...
| ② DOCX→PDF | 🔧 | **不穩** | 同檔三轉 8/9/9 | 根因未查明；⑤ 止血中 |
| ③a 判大題 | 🤖 | 穩（樣本小） | L0072×3 → 9/9 全同 | 再測別的課型 |
| ③b 頁碼定位 | 🔧 | 穩 | 1426/23/0 | — |
| ③c 橋接 | 🔧 | 穩 | 三次輸入 → 同一輸出 | ✅ 已接進 overview skill |
| ④ 派工單 | 🔧 | 穩 | `--check` 在 CI | — |
| ⑤ 派工前對帳 | 🔧 | **半套** | 只比頁數；同頁數版面重排放行（實證） | 改內容錨點（§0.5 Q1） |
| ⑥ 飛機抽取 | 🤖 | **未量** | 只跑過 1 課 1 次 | 一致率要量 |
| **⑥b 見證對帳** | 🔧 | **穩** | 校準 12/12 · **147 課全跑：模組層 377 對 / 42 驗不了** · 抓到真缺陷 #2867 | ✅ 飛機鐵律 2b 已接 |
