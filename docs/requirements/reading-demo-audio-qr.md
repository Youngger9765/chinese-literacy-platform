...
⚠️ **判準是資料，不是年級（2026-08-31 改，#3011）**：

| 這一節 | 出不出 QR |
|---|---|
| 有課文（`has_full`）| 出**全文**碼 |
| 有念順順段（`has_key`）| 出**段落**碼 |
| 兩者皆無 | 不出 —— 碼掃開會是一頁沒有東西唸的紙 |

原本這裡寫的是年級差異（4–7 年級兩個碼、8–9 年級只有段落碼）。那條規則的理由是
**技術性**的：`build_demo_reading.plan_demo_audio` 只替 4–7 年級批次產
`demo-reading/{id}/whole.mp3`，替其他年級印碼會指向一個永遠不存在的檔。

那個限制已經不存在了 ——

- `build_demo_reading.py` 隨 #2916 一起刪除（commit `6894dda73`）
- 前端沒有任何地方抓預產 mp3（`grep demo-reading` 只剩註解與測試字串）
- QR 指向 `/learn/{id}/{step}`，訪客頁與登入頁走**同一條**即時 TTS

而年級這個判準另有破法：`grade` 從 API 來是**字串**，179 課裡有 27 課的值不是數字
（`文言文` 12、`品格教育` 15），`Number.parseInt` 回 NaN → 一律不給全文碼。
2026-08-31 明珠老師回報體育生品格那批掃不到全文 QR，就是這個。

> Owner 2026-08-31：「喔 那應該可以生成 / **只要有課文就可以生成**」

改動效果（實測，全庫）：全文 QR **105 → 175 列**、段落 QR **149 → 154 列**。
規則的唯一實作在 `frontend/src/components/qr/lessonQr.ts::hasWholeTextToRead`。
