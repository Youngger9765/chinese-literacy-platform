/**
 * useKeyPassageReadingSession — STT/recording lifecycle for KeyPassageReading.
 *
 * Issue #1880: Extracted from KeyPassageReading.tsx to isolate STT complexity.
...
/**
 * Issue #2321 — Hallucination sanity gate for KeyPassageReading scoring.
 *
...
interface UseKeyPassageReadingSessionProps {
  fullText: string;
  token: string | null;
...
interface UseKeyPassageReadingSessionReturn {
   ...
   * Kept for interface compatibility with KeyPassageReading.tsx.
   */
...
export function useKeyPassageReadingSession({
  fullText,
  token,
  storyId,
  dbSessionId,
  ...
  onResultReady,
  onAudioSaved,
}: UseKeyPassageReadingSessionProps): UseKeyPassageReadingSessionReturn {
  ...
  const submitReading = useCallback(async () => {
     ...
     * Shared validateRecording() — same constants as ParagraphReading.
    ...
    const _evaluate = (finalTranscript: string, capturedAudioBlob: Blob) => {
      ...
      if (token && dbSessionId != null) {
        saveReadingAudio(capturedAudioBlob, dbSessionId, undefined, token)
          ...
          .catch((err) => console.warn('[KeyPassageReading] saveReadingAudio error:', err));
      }
