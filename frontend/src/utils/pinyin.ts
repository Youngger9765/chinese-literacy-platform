...
// Compact encoding: pinyin → characters that share that reading.
// We invert this at load time into a char → pinyin map.
const PINYIN_GROUPS: Record<string, string> = {
  ...
  bai: '百白擺拜敗柏佰襬稗',
  ban: '半班般辦版搬伴扮瓣拌絆頒斑板闆扳粄',
  bang: '幫棒綁膀榜傍磅蚌邦',
  bao: '包報保抱暴薄寶飽爆剝雹堡褒豹鮑胞苞煲',
  bei: '北被背杯備悲倍輩碑貝卑悖盃焙憊狽蓓揹',
  ben: '本笨奔苯',
  beng: '繃崩蹦迸泵',
  ...
  zun: '尊遵樽鱒',
  zuo: '做作坐座左昨佐',
  // Extended from 方大哥's learning-to-read-chinese SQLite (3666 chars)
  miu: '謬',
  n: '嗯',
  nve: '虐瘧',
  pou: '剖',
...
// Build inverted map: character → pinyin (toneless).
const charToPinyin = new Map<string, string>();
for (const [py, chars] of Object.entries(PINYIN_GROUPS)) {
  for (const ch of chars) {
    // First mapping wins (most common reading).
    if (!charToPinyin.has(ch)) {
      charToPinyin.set(ch, py);
    }
  }
}
