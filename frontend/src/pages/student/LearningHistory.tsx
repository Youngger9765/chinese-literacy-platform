...
const LearningHistory: React.FC = () => {
  ...
  const loadSessions = useCallback(async (off: number) => {
    ...
    setError('');
    try {
      const data = await fetchLearningSessions(token, { limit: LIMIT, offset: off });
    ...
    <div className="flex-1 overflow-y-auto p-6 sm:p-8">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        {!isLoading && !error && sessions.length > 0 && (
          <div className="space-y-3">
            {sessions.map((s) => {
              const hasDialogue = s.current_step >= 3; // reached comprehension step
              return (
                <div
                  key={s.id}
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex flex-col gap-2 shrink-0">
                      {hasDialogue && (
                        <button
                          onClick={() => navigate(`/sessions/${s.id}/dialogue`)}
