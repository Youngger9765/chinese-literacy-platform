...
import { useKeyPassageReadingResultPersistence } from '../../hooks/useKeyPassageReadingResultPersistence';
...
const KeyPassageReading: React.FC<KeyPassageReadingProps> = ({
  ...
  } = useKeyPassageReadingResultPersistence({
    storyId: story.id,
    token,
    userId: user?.id,
    storageKey,
  ...
  const {
    ...
    stopTtsAll,
    isTtsPlaying,
  } = useKeyPassageReadingTtsQueue({ storyContent: readingContent });
   ...
   *      Declared AFTER useKeyPassageReadingSession so `audioRecorder` exists when
  ...
  /* ── Derive control state for KeyPassageReadingControls ─────────────────── */
  ...
  return (
    ...
    >
        ...
        <div className={`mx-auto px-6 md:px-16 pt-4 ${result ? 'max-w-6xl' : 'max-w-4xl'}`}>
          ...
          {result && (
            <div ref={resultRef} className="space-y-6 mt-4">
              {/* Stars + encouragement + transcript + audio (Issue #1960 — KeyPassageReadingScoreCard) */}
              ...
              {/* 朗讀結果 (Issue #1960 — same style as ParagraphReading ParagraphCard) */}
      ...
      {/* Issue #2609: TTS Web Speech API fallback alert — must not be silent.
          ...
          `absolute bottom-48` (not in-flow `mt-4`): KeyPassageReadingControls is a
          ...
          ParagraphReading.tsx sibling banner does keeps it visible regardless of
          passage length. */}
      {tts.isTtsDegraded && (
      ...
      {/* ── Fixed bottom CTA (Issue #1960 — KeyPassageReadingControls) ────── */}
...
export default KeyPassageReading;
