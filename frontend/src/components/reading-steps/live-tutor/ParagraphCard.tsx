      ...
      {paragraphSummary && (
        <div className="mt-8 p-6 rounded-2xl bg-surface-container-low space-y-4">
          ...
          {isCurrentIdx && (() => {
            ...
            // Rule 2: if > half the sentences failed → suggest redo, always show next-segment.
            // Advancement is never blocked by score — retry is advisory only. (#1318)
            if (failedSentences.length > totalRetryable / 2) {
              const canAdvanceRule2 = true;
              return (
          ...
          {retrySentenceIdx === undefined && (() => {
            // Advancement is always allowed — retry suggestions are advisory only. (#1318)
