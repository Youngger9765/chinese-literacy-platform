...
// jsdom has no navigator.mediaDevices; ParagraphReading pre-warms the mic on mount (rejection
...
function renderCompletedParagraphReading() {
...
describe('ParagraphReading 「再讀一次」(#2532)', () => {
  ...
  it('clicking 再讀一次 invokes the cross-layer full reset (onResetTutor)', () => {
    const { onResetTutor } = renderCompletedParagraphReading();
  ...
  it('clicking 再讀一次 returns the page to the unread state (朗讀對照 gone)', () => {
    ...
    // (見 ParagraphReading.tsx) + reducer 單元測試 useParagraphEvaluation.test.ts『CLEAR_FOR_PARAGRAPH …
