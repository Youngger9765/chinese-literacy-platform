...
import { LineResult, ParagraphSummaryData } from '../paragraphReadingTypes';
  ...
  const evaluateAndRespond = useCallback(
    async (
      ...
      if (isEvalConsoleEnabled()) {
        console.log('[ParagraphReading] STT', {
          lineIndex: lineIdx,
          targetText,
          webSpeech: rawStt,
      ...
      void (async () => {
        ...
        try {
          ...
          if (isEvalConsoleEnabled()) {
            console.log('[ParagraphReading] LLM 評估', {
              localBaseline: {
                tier: localTier,
                matchRate: localMatchRate,
                cpm: localCpm,
          ...
          if (msg !== 'gemini_timeout') {
            console.warn('[ParagraphReading] Gemini eval failed, local result stands:', err);
          }
        }
      })();
    },
