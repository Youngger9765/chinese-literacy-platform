/**
 * KeyPassageReadingScoreCard — Star encouragement + transcript + audio playback (Issue #1960).
 *
 * Extracted from KeyPassageReading.tsx result section:
 *   - Star encouragement (5/4/3/2 stars based on matchRate)
 *   - 你說的 streaming transcript (optional)
 *   - 重聽錄音 audio playback (optional)
 *
 * Note: 朗讀結果 color display is in KeyPassageReadingFeedbackPanel
...
export function getKeyPassageReadingEncouragement(matchRate: number): { stars: number; text: string; color: string } {
...
export interface KeyPassageReadingScoreCardProps {
  result: {
    matchRate: number;
    feedback: string;
...
const KeyPassageReadingScoreCard: React.FC<KeyPassageReadingScoreCardProps> = ({
  result,
  streamingTranscript,
  audioUrl,
}) => {
  const enc = getKeyPassageReadingEncouragement(result.matchRate);
...
export default KeyPassageReadingScoreCard;
